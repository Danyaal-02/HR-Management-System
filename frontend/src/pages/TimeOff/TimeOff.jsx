...
function TimeOff() {
  ...
  // ===== Admin Data =====
  const pendingLeaves = useMemo(() => getPendingLeaves(), [leaveRequests, getPendingLeaves]);
  ...
  const calendarData = useMemo(() => {
    ...
    const empLeaves = leaveRequests.filter(lr => 
      lr.employeeId === user.id && lr.status === 'approved'
    );
  ...
  const getLeaveTypeClass = (type) => {
    switch(type) {
      case 'Paid Leave': return 'timeoff-type--paid';
      case 'Sick Leave': return 'timeoff-type--sick';
      case 'Unpaid Leave': return 'timeoff-type--unpaid';
      default: return '';
    }
  };
      ...
      {showForm && (
        <div className="timeoff-form-card" id="timeoff-apply-form">
          ...
          <form onSubmit={handleApplyLeave}>
            ...
            <button type="submit" className="timeoff-form-submit" id="timeoff-submit-btn">
              Submit Request
            </button>
          </form>
        </div>
              ...
              {myLeaves.map(lr => (
                <div key={lr.id} className="timeoff-history-item">
                  <div className="timeoff-history-item__top">
                    <span className={`timeoff-type-badge ${getLeaveTypeClass(lr.type)}`}>{lr.type}</span>
                  ...
                  <div className="timeoff-history-item__dates">
                    ...
                    {lr.startDate} → {lr.endDate} ({lr.days} day{lr.days > 1 ? 's' : ''})
                  </div>
          ...
          {['all', 'pending', 'approved', 'rejected'].map(filter => (
            ...
            >
              ...
              {filter === 'pending' && pendingLeaves.length > 0 && (
                <span className="timeoff-filter-badge">{pendingLeaves.length}</span>
              )}
            </button>
          ))}
        </div>
                  ...
                  <tr key={lr.id} className={lr.status === 'pending' ? 'timeoff-row--pending' : ''}>
                    <td>
                      <div className="timeoff-emp-cell">
                        {emp?.profilePicture ? (
                          <img src={emp.profilePicture} alt={emp?.name} className="timeoff-emp-avatar" />
                        ) : (
                        ...
                        <div className="timeoff-emp-info">
                          <span className="timeoff-emp-name">{lr.employeeName}</span>
                          <span className="timeoff-emp-dept">{emp?.department || ''}</span>
                        </div>
                      </div>
                    </td>
                    <td>
