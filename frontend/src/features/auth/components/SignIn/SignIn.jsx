...
function SignIn() {
  ...
  const onSubmit = (data) => {
    loginMutation.mutate(
      { login_id: data.loginId, password: data.password },
      {
        onSuccess: (res) => {
          if (res.success) {
        ...
        onError: (err) => {
          ...
          setError('loginId', { type: 'manual', message: msg })
        },
      }
    )
  }
    ...
    <div className="min-h-screen bg-bg-dark flex items-center justify-center relative overflow-hidden p-5">
      ...
      <div className="relative z-10 w-full max-w-[900px] min-h-[550px] bg-bg-card/70 backdrop-blur-md border border-white/10 rounded-2xl flex overflow-hidden shadow-card md:flex-row flex-col max-md:max-w-[440px] max-md:min-h-0">
        ...
        <div className="flex-1 flex items-center justify-center p-12 max-md:p-8 max-sm:p-5">
          <form className="w-full max-w-[380px]" onSubmit={handleSubmit(onSubmit)} id="signin-form">
            ...
            <div className="flex flex-col gap-5 mb-7">
              <div className="flex flex-col gap-1.5">
                ...
                {errors.loginId && (
                  <span className="text-[0.78rem] text-status-error flex items-center gap-1 mt-1 pl-1">{errors.loginId.message}</span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {errors.password && (
                  <span className="text-[0.78rem] text-status-error flex items-center gap-1 mt-1 pl-1">{errors.password.message}</span>
                )}
              </div>
            </div>
